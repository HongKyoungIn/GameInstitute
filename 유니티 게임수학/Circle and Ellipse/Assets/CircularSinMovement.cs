...
public class CircularSinMovement : MonoBehaviour {
    ...
    private float rotationFrequency = 0.2f; // 초당 회전 수
    private float ellipsedTime; // 경과 시간

    private float sinFrequency = 8f;
    ...
    private void Update() {
        ...
        float sinOffset = sinAmplitude * Mathf.Sin(t * Mathf.PI * 2 * sinFrequency);
        Vector3 normal = (transform.position - center).normalized;
        transform.position += normal * sinOffset;
    }
}